...
5. During the hour, a calm focus screen shows the activities as a simple checklist.
6. Afterwards, the user can mark the hour complete or move it—without guilt or streak pressure.

## First iPhone version

- Weekly planner for setting a time and activities for each day.
- A visual hourglass representing today’s Happy Hour.
- Custom activities and a short list of suggestions.
- Local notifications when Happy Hour starts.
- A focused, low-pressure checklist for the active hour.
- Optional calendar integration so Happy Hour appears as a real appointment.
...
- A near-black background and one large, visual hourglass as the main focus.
- The selected weekday and Happy Hour time sit quietly above the glass.
- Swipe left and right to move between Monday and Sunday, with a subtle day indicator.
- A clear edit button opens the weekly plan for that day.

### Activities inside the hourglass

- Every activity is shown as a horizontal row inside the hourglass.
- Adding an activity makes room for another row; the layout can hold up to ten activities.
- Each row has a muted, warm colour so the hourglass feels visual rather than like a task list. Suitable colours include dusty sage, blue-grey, lavender, sand, and soft terracotta.
- The row shows a short activity name, such as “Cold shower.”
- A small, unobtrusive info button on the row opens optional details: a description, personal notes, or helpful tips. This keeps the main view uncluttered while making richer context available when wanted.
...
- **Calendar:** include a simple “Add to Calendar” action. It creates the selected Happy Hour as a calendar event; there is no ongoing calendar sync in version one.
